...
uniform sampler2D combTex;
uniform sampler2D blurTex;
uniform sampler2D softlightTex;
uniform float combIntensity;
...
void main() 
{
    vec4 combColor = texture2D(combTex, uv) * combIntensity * intensity;
    vec4 srcColor = texture2D(srcTex, uv);
    vec4 blurColor = texture2D(blurTex, uv) * blurIntensity * intensity;
    vec4 softlightColor = texture2D(softlightTex, uv);
    combColor += blurColor;
    srcColor.rgb = blendFunc(srcColor.rgb, softlightColor.rgb, intensity, 21);
    combColor = 1. - (1. - srcColor) * (1. - combColor);
    gl_FragColor = clamp(combColor, vec4(0.), vec4(1.));
